...
const double VELOCITY_CHANGE_DECEL = 0.3; //.35 worked
const double VELOCITY_CHANGE_ACCEL = 0.3; //.2 worked

//SUPPLIED
//start in lane 1. 0 is left, 2 is right
int lane = 1;
int oldLane = 1;
double ref_vel = 0; //mph
bool laneChanging = false; //is the car in the process of changing lanes
double lookingForwardMeters = 75.0;
double lookingBackwardsMeters = 40.0;
double accel_sTarget = 0.0;
		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2') {
			...
			if (s != "") {
				...
				if (event == "telemetry") {
					...
					accel_sTarget = 100.0;
					vector<vector<double> > lane0CarsOfInterestAhead; //will hold all of the cars that i care about 300m ahead
					vector<vector<double> > lane1CarsOfInterestAhead;
					vector<vector<double> > lane2CarsOfInterestAhead;

					vector<vector<double> > lane0CarsOfInterestBehind; //will hold all of the cars that i care about 60m behind
					...
					for (int i = 0; i < sensor_fusion.size(); i++)
					{
						...
						if (d < (car_d + 2) && d >(car_d - 2) && !laneChanging) //we need to slow down now
						{
							...
							else if ((check_car_s > car_s) && ((check_car_s - car_s) < 50)) //crude came up on slow car //20 worked and 25
							{
					...
					else if (!too_close && ref_vel < TARGET_SPEED) //49.5
					{
						...
						{
							ref_vel += (accel_sTarget - car_s) * 0.006;
						}
						else
